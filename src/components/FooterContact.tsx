...
export default function FooterContact({ socialLinks }: FooterProps) {
  const [copied, setCopied] = useState(false);

  const email = socialLinks?.email || "sisshmueaung@gmail.com";
  const github = socialLinks?.github || "https://github.com/Sisshmue";
  const linkedin =
    socialLinks?.linkedin || "https://www.linkedin.com/in/siss-hmue-aung-313a79167/";
  const whatsapp = socialLinks?.whatsapp || "https://wa.me/+66658022587";
    ...
    >
      ...
      <div className="pt-2 sm:pt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3 font-mono text-[10px] sm:text-[11px] uppercase tracking-[0.2em] text-slate-400">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[#0055ff] font-semibold">Open for opportunities</span>
        </div>
      </div>
      ...
      <div className="my-auto py-6 sm:py-10 flex flex-col items-start justify-center">
        ...
        <div className="flex flex-col sm:flex-row sm:items-baseline gap-3 sm:gap-4 mb-5 sm:mb-8">
          <a
            href={`mailto:${email}`}
            data-cursor="Email"
            className="group relative text-2xl sm:text-4xl lg:text-5xl font-extrabold uppercase text-[#0a0a0c] tracking-tight font-heading inline-block"
          >
            <span>{email}</span>
      ...
      <div className="w-full select-none pt-6 sm:pt-8 border-t border-slate-200">
        ...
        <div className="mt-4 sm:mt-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 font-mono text-[10px] uppercase tracking-[0.2em] text-slate-400">
          <div>© {new Date().getFullYear()} Siss Hmue Aung • Bangkok, Thailand</div>
